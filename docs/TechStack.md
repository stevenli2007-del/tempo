...
**学生 token 的过期时间是强制必填字段**（bCourses 弹窗 Expiration date + Expiration time 均带 `*`，2026-09-04 P0-2-1b 实测），上限 **90 天**（同次实测，弹窗原文 "Maximum expiration is 90 days."）。

- ✅ `expires_at` 使用**用户实际填写的过期时间**（必填，库内 NOT NULL）
- ✅ 到期提醒基于该字段计算（窗口 T-14 / T-7 / T-3 / T-1 / 当天），而非固定天数
...
| 2026-09-04 | **推翻**上一条：P0-2-1b 实测过期上限为 **90 天**（弹窗原文），过期强制必填（无 null fallback）。§7.2 同步重写（删"政策在变化/收紧"措辞与"❌ 不要写死 90 天"条目，因实测就是 90 天；保留"用 expires_at、不要写死固定天数"的本质告诫）。 | P0-2-1b 实测（Steven 提供截图） |
