...
> 🔴 **2026-09-18 更新**：`Phase-0-MVP.md` 已瘦身为**薄索引**（原 1914 行 → 227 行），逐卡详情搬到 `docs/cards/`。
> 按下面三步读，**不要再一次读 6 份文档**（那是拆分前的做法，开局上下文会被撑爆）：
> 1. 读 `Phase-0-MVP.md`（薄索引）→ 拿「当前卡 / 上一张 / 下一张 / 卡表状态」。
> 2. 读**当前那一张卡** `docs/cards/P0-3-XX.md`（内含做什么 / 改哪些文件 / 关键约束 / 验收命令 / 交接点）。
> 3. 按 `docs/cards/README.md` 里该卡的**依赖清单「按需」**读其余文档 —— 不是全读。
>
> - **首次开工**：完整读一遍本文档 + `TechStack.md`（尤其第 2 节的版本锁定矩阵）。**这两份恒定必读**，不逐卡重复列。
> - **架构级 task**（新增抽象层、改数据模型、改 API 契约等）才需要回到对应实现文档（`TechStack.md` / `Database.md` / `API-Contract.md`）核对细节。
> - **`// ?` 扫描收窄**：见 §6「`// ?` 提问标记」—— 只扫本卡涉及的 `lib/` 子目录。
>
...
（`// ?` 会被 Better Comments 高亮成蓝色。）**下次会话开工前扫一遍并统一作答**，再开始新卡 ——
但**不要再全仓扫**（2026-09-18 改）：`grep -rn "// ?" lib/<本卡涉及的子目录>/`，
收窄到当前卡会碰的目录。全仓扫会把几十个无关文件的提问一起端上来，开局上下文就是这么爆的。
...
22. 🔴 **录屏做演示素材：隐私闸门是「裁切」，不是「打码」；而"改 DOM"那一招够不着动态浮层。**
    P0-3-32 的教程动图（2026-09-20）一次踩齐两个坑：
    ① Canvas 的账号浮层**是点开之后才渲染的** —— 先跑脚本把姓名改成 `Test Student`、再把头像
       `filter: blur()` 掉，对**当时还不在 DOM 里**的元素无效：页面静态部分的姓名改掉了，
       浮层里那个原样留着。**"先清理再录"只对已经渲染出来的东西成立。**
    ② Settings 页右栏「Ways to Contact」就是登录邮箱，它**从第 0 帧就在**，而且和要演示的对话框
       **横向重叠**（对话框右缘 ≈2133px 落在右栏 1882–2520px 之内）→ 按 x 裁切会在切掉邮箱的
       同时把「Generate Token」按钮切掉。
    **正解**：找出邮箱的**纵向**区间（本例 y 442–602），把裁切**上边界**定在它下面（y 640）——
    页面只会向上滚动，那块内容**永远不可能**再落回可见区。左边界同样不能用"看起来差不多"的数：
    取 670 会把「+ New Access Token」按钮（从 x≈482 开始）整块切没，**必须放大一块原始分辨率看边界**。
    **流程（缺一不可）**：抽帧逐张看 → 定 crop → 编码 → **再从产物抽帧复核一遍**。
    复核产物而不是源文件 —— 源文件干净不代表编码参数选对了。
    **判据**：凡"后期能修"的说法，先回答「它会不会出现在**某一帧的中间**」：
    只在头尾出现可以裁时间轴，出现在中间就只能重录。
    **顺带**：能用 `type="password"` 挡住的输入就别靠肉眼确认（本项目 `canvas-connect-form.tsx`）——
    靠输入框类型保证，比逐帧看可靠一个数量级。
    ③ **别在缩略接触表上给素材下结论**（同日回改）：我第一版把 card2 那条片子判成"内容不对
       （演的是无障碍设置弹窗）"并弃用；放到**原分辨率**一看，它演的正是这张卡要教的
       「点左上角头像 → 菜单里的 Settings」，而 Accessibility Settings 只是那个弹层里的**一个分区**。
       **判"这张片子能不能用"必须看原分辨率**；缩略图只够回答"有没有一眼可见的泄露"。
    ④ **教程文案里「在哪里点」是事实，写完要拿录屏对一遍**：原稿写「左侧菜单**底部**找到 Account」
       +「**右上角**头像同样能进 Settings」，实际 Account 就在左侧栏**最上方**（Cal 校徽正下方）。
       两份素材（文案 / 画面）互相打架时，用户先信画面 —— 然后认为整个产品是抄来的模板。
       **文案里每个方位词都是可验证断言**，不是修辞。

### 10.2 坑索引（细节在各自文档）

| 坑 | 一句话 | 权威位置 |
|---|---|---|
| 录屏素材的隐私复核 | 裁切 > 打码；"改 DOM"够不着动态浮层；**产物必须再抽帧复核一遍** | 本节 10.1 第 22 条 |
...
| ~~🔴 Canvas 文件的外链要**自己拼**（`url` 需 Bearer，浏览器点开 401）~~ **[已作废：见下一条]** | 2026-09-18 实测推翻："需 Bearer"是错的 —— 那个 `url` **不带任何 Authorization 头也返回 200 与文件本体**，它是**能力凭据**不是链接。结论随之反转：不是"自己拼一个可点链接"就够了，而是**那个 `url` 绝不能落库/下发**。保留本行只为留痕，**不要照它做** | `lib/canvas/files.ts` `filePreviewUrl()` |
...
| 🔴 探针里**换过 token 的那个 client 不能再用来"匿名读"** | 在 `anon` client 上先 `verifyOtp()` 换会话、再拿它读数据 → 报出「anon **读到 1 行** —— RLS 没在拦」。真相是 `verifyOtp` 把 session 写进了**那个 client 自己的**内存 auth store，此后它的请求自动带上 `Authorization: Bearer <用户 token>` —— **是探针在冒充登录用户，数据库完全无辜**，差一点就据此去改一条**本来是对的** RLS 策略。**假阳性比漏报更危险：它让人去修没坏的东西。** 正解 = 换 token 的 client 与读数据的 client **拆成两个**，并加一条 **raw `fetch`（只带 `apikey`、不带 `Authorization`）** 作为第二条腿。P0-3-23 实测 | 本节 10.1「RLS 有两半」、`scripts/probe-practice-tests-schema.ts` |
| 🔴「把两份材料认成一对」的判据必须是**同一性**，不是**相似度** | 试卷↔答案、邮件↔附件、发票↔订单、去重归并都适用。按相似度排会让 `PracticeMidterm1KEY_F23` 配给 `PracticeMidterm2_F23`（同门课、名字高度相似、年份相同）—— **一个看起来完全合理、却彻底错误的配对**，用户拿到的是"答案对不上题"的卷子。正解：先算**剥掉 KEY 词后的指纹**并要求**指纹相等**才算候选，再在候选里排序（同目录 > 同后缀 > 名字短）；**指纹为空（文件名整个就是 KEY 词）时放弃配对**，不给一个瞎猜的结果。口诀：**先要求"这两个确实是同一件事"，再在候选里挑最好的那个**。P0-3-23 实测 | `lib/practice-test/pairing.ts` `normalizeStem()` |
| 🔴 多输入依赖的任务，某个输入拿不到时应当**整体失败**，不要降级成"只有一半"的产物 | 答案文件读不出来（PDF 无文字层 / 下载失败 / 抽不出文本）时**整张卷子判失败**，而不是产出一份"没有答案的卷子" —— 降级版会让用户以为**那道题本来就没有答案**。**假信息比缺失更糟，它把系统的失败伪装成数据的属性。** 判据问句：**"这个输入换掉/缺失，用户区分得出来吗？"** 区分不出来 → 必须整体失败。P0-3-23 实测 | `lib/practice-test/generate.ts`、本节 10.1 第 17 条（"不知道" ≠ "未完成"）同源 |
| 🔴 **站内路径也要守卫** —— `//host` 是**协议相对 URL** | `payload` 是 jsonb，一个路径字段就能塞进 `//evil.com/x`：浏览器会当 `https://evil.com/x` 处理，而它和 `/courses/…` **只差一个字符**，评审时几乎看不出来（`/\evil.com` 同理 —— 部分浏览器把反斜杠当斜杠）。所以 `readInternalPath` 的判据是「以**单个** `/` 开头」。另一条：**别把站内路径喂给只放行 http(s) 的守卫**（判 null → 链接凭空消失、零报错），站内入口与外链刻意分成两个字段。P0-3-23 | `lib/messages/view.ts` `readInternalPath()` |
| 🔴 **无凭证探针在「受保护前缀」下的页面路由上会失效** | `proxy.ts` 的 `PROTECTED_PREFIXES`（`/dashboard` / `/settings` / `/messages`）用**前缀匹配**（`pathname === p \|\| pathname.startsWith(p + '/')`），且 proxy 跑在 Next 路由匹配**之前** → `/messages/随便什么/再深一层` **也返回 307 → /login**。它看起来"证明路由存在"，其实什么都没证明（对照组必须选**不在任何受保护前缀下**的路径，如 `/definitely-not-a-page-xyz` → 404，那才是真对照）。**更硬的一条**：落在受保护前缀下的页面，**无凭证探针根本证不了它存在**（恒 307）—— 这类路由的上线判据只能靠 ① `gh api .../commits/<sha>/status` = success ② **build 日志里的路由表**（`npm run build` 输出的那串）③ 带真实会话访问。⚠️ 与 `API-Contract` 那套「404=没上 / 401=已上线」**不通用**：API 路由不走登录重定向，那套在那里才有效。P0-3-23 实测踩到 | `proxy.ts`、`lib/supabase/proxy.ts` `PROTECTED_PREFIXES`、本节 10.1 |
| 中文文案里的 **ASCII 直引号**会打断字符串字面量 / 触发 lint | 三种形态：① **字符串字面量**被从中间打断（esbuild 报 `Expected ")"`，硬故障）；② 只在**注释**里 → **假警报**（grep 会命中，其实无害）；③ **JSX 文本节点**（`{}` 内的 JS 表达式不算）→ `react/no-unescaped-entities` 报错。**写作习惯：中文文案（JSX 文本 / 字符串字面量 / 注释一律）用「」，ASCII 引号只留给代码语法。** ⚠️ 2026-09-13 已记过、2026-09-18 P0-3-23 **又踩一次** —— **"记忆里记过"不等于"写代码时会查"** | `components/courses/practice-paper.tsx`、本节 10.1 |
| 🔴 `numeric(p, s)` 列与**未定标浮点**比相等 → 差量判定永不收敛 | 症状是同步每轮都报 `updated N` 而数据其实没变（实测 `submission.score = 9.923076923076923` 存进 `numeric(10,2)` 读回 `9.92`）。后果不只是多余写入：**`updated_at` 被无意义刷新**，失去"这条数据什么时候真变过"的意义。**只在比较端四舍五入不够** —— 落库的仍是未定标值，下一轮要靠"数据库的舍入规则与 JS 一致"才收敛，而两者在边界值上并不一致（JS `Math.round` 对 `-2.5` 给 `-2`，Postgres `numeric` 给 `-3`）。正解：**写入与比较共用同一个已定标的值**（`roundToScale()`），收敛由构造保证，不依赖任何舍入约定。**验收 = 连跑两轮第二轮全零**，不是"我改了比较逻辑" | `lib/numbers.ts` `roundToScale()`、`lib/sync/canvas-tasks.ts` `toCanvasTaskFields()`、`scripts/regress-sync-idempotent.ts` |
| 🔴 `npm run <script> --live` **不会把参数传给脚本**，静默退化成不带参数的那条路径 | npm 把 `--live` 当**自己的** cli config 吃掉，只 warn `Unknown cli config "--live"`（**不报错**）→ `process.argv.includes('--live')` 为假 → **真账号那一段根本没跑，而退出码仍是 0、断言全绿**，唯一区别是末行文案 `（纯断言；加 --live 跑真账号）`。正确写法 **`npm run <script> -- --live`**（`--` 之后的才透传）。通用规则：**"加了参数才跑真链路"的验收脚本，判据不能只看退出码** —— 要认末行是否写了"含真账号…"。2026-09-18 P0-3-28 复验时实测踩到 | `package.json` scripts、`scripts/regress-sync-idempotent.ts` `main()` |
| 🔴 **上线判据遇到同形动态段路由时，"没上"表现为 405 而非 404** | 无凭证探针打到尚未部署的 `/api/v1/messages/drift`，被旧部署已有的动态段路由 `messages/[id]`（只声明 GET/PATCH）接住 → 返回 **405**（路由存在但方法不匹配），连续轮询第 3 次才落到 401 的真实已上线态。**只认 404 会把"还没部署"误读成"路由被删了"**。判据按"这条路径在旧部署会先撞上谁"定：有同形动态段路由时表现为 405、无落点时才是 404。与「页面路由在受保护前缀恒 307」是同一类问题的不同面。2026-09-18 P0-3-20 复验时实测 | `proxy.ts`/API 路由表、本节 10.1「无凭证探针在受保护前缀下的页面路由上会失效」 |
| 箭头函数「`=> ({ … })` **不带分号** + 紧接着一个**裸块 `{`**」→ TS 解析器误报 | 症状：`@typescript-eslint/parser` 报 `Identifier expected. 'null' is a reserved word that cannot be used here`，而**位置指向对象体内部某一行的 `null`**（完全误导），`tsc --noEmit` 却**全绿**（真编译器不这么解析）→ 表现为「tsc 过、eslint 挂」。根因是解析器把 `({…})` 后的 `{` 当成块语句，于是 `b: null` 被当标签语句、`null` 落进标识符位置。修法二选一：① 给 `})` 后补分号（本仓库是无分号风格，仅作为应急）② **把那个 helper 写成函数声明**（推荐，最干净）。⚠️ 回归脚本里大量用「裸块分组 + 行内 helper」的写法，极易撞上 | `scripts/regress-course-updates.ts`（P0-3-29 实测） |
| 🔴 文档拆分的**逐字承载区**不许插注记、不许改写 | `docs/Phase-0-MVP.md` 的**卡表段**（基线 `f625a5a` L1099–1135）与 **23 张 P0-3 卡的卡面**是「只搬家不重写」的逐字承载区：改一行 → `scripts/verify-phase0-docs-split.py` 的 ① 报「有 63 行基线内容没有任何落点」（**一行改写会让其后所有区间整段失配**）、② 报该卡「逐字命中=False」。**「当前卡」这类过程状态只能写进度指针表**（薄索引里 09-18 新写的压缩段 = "新增内容"，不受约束）；卡表行的状态列要改必须先往 `DECLARED_EDITS` 登记（先例 = P0-3-25b 那条）。⚠️ **拆分之后新开的卡（29~34）不在承载区，随便改** —— 踩坑的只会是**老卡**。2026-09-21 P0-3-34 收尾时实测踩到 | `scripts/verify-phase0-docs-split.py`（`RANGES` / `DECLARED_EDITS` / `DECLARED_NEW_CARDS`）、本节 10.1 |

| 🔴 记「第一次发生」的表：**幂等交给主键，写入用先查后插，别用 `upsert`** | 「某件事已经发生过」这类表（音符、已读、已发）若靠"先查再插"守幂等，两个并发请求会都查到"没有"然后都写进去 —— 幂等必须是**主键**的性质（如 `(user_id, task_id)`）。而写入**不能**用 `upsert`：`on conflict do update` 会把已存在行的 `awarded_at` 刷新成现在，「首次发生于」这个语义就被下一次刷新抹掉了。P0-5-4 实测 | `supabase/migrations/20260928000000_note_awards.sql`、`lib/notes/store.ts` `awardNotesForDone()` |
| 🔴 PostgREST `or()` 串里各项是**析取** —— 用 `neq` 表达「不在集合内」会退化成**恒真** | 想排除「提交态 ∈ {submitted, pending_review, graded}」而写成 `or=(submission_state.neq.submitted,submission_state.neq.pending_review,…)`：一条 `S='submitted'` 的行**因为满足 `neq.pending_review` 而被保留** → filter 静默失效（2026-09-23 实测：加完仍有 16 条该排掉的历史）。根因：`NOT(S=a OR S=b)` 展开是**合取**（`S≠a` **且** `S≠b`），而 `or()` 只有析取。正解 = 改用**正向 `eq` 枚举补集**（`S IS NULL OR S=<未完成态>…`），且补集**从全量枚举减出来**而不是手抄。同源陷阱：`x NOT IN (...)` 遇 `x IS NULL` 求值为 **NULL**（不成立）→ NULL 行**整批被排除**，所以补集里必须显式带 `is.null` 一支。**多次 `.or()` 调用之间是 AND**（实测：两条各剩 16 / 7，合起来 0），可拿它合成交集。🔴 通用规则：**SQL 侧的 filter 必须打真库验** —— 它被忽略时**不报错**，`tsc`/`eslint`/`build`/纯函数回归**全绿**，只有"行集没变"能暴露。P0-3-35 实测 | `lib/tasks.ts` `doneHistoryFilters()`、`scripts/probe-task-window.ts` |
| 🔴 列表取数「只有时间**上界** + 按日期**升序**」→ **历史会把未来挤出结果集** | 症状：总览页 7 天日历全空、待办只剩一场考试，而同步明明是好的（实测 50 条里 35 条是已交作业、未来任务 **0** 条）。根因 = 上界只挡未来、升序让最早的历史排最前，`limit` 名额被历史吃光。**只调大 limit 是治标**（下学期又爆）：正解是给**已完成**的行单独设下界，而**未完成的行（含逾期未交）一律不受下界约束** —— 给它们设下界等于帮用户逃避，与「不隐藏问题」冲突。改完若窗口内条数的**含义**变了（如"已完成 49"变成"近 7 天 12"），**UI 文案必须跟着改**，否则用户只看到数字掉了不知道为什么。P0-3-35 实测 | `lib/tasks.ts` `loadTasks({ historySince })`、`app/(routes)/dashboard/page.tsx` |

---

*创建：2026-09-01 ｜ 最近更新：2026-09-23（**§10.2 新增一行**：文档拆分的「**逐字承载区**」——`docs/Phase-0-MVP.md` 卡表段（基线 L1099–1135）与 23 张**老卡**卡面 —— **不许改写、不许插注记**：改一行会让其后所有区间整段失配，`verify-phase0-docs-split.py` 的 ① ② 双红。「当前卡」这类**过程状态只写进度指针表**（新增内容区）；卡表状态列要改必须先登记 `DECLARED_EDITS`。P0-3-34 收尾时实测踩到，三把文档闸门已复跑全绿）。此前 2026-09-19（**§10.2 新增一行**：箭头函数「`=> ({ … })` 不带分号 + 紧随裸块 `{`」会让 TS **解析器**误报 `Identifier expected. 'null' is a reserved word` 且位置指错行，而 `tsc` 全绿 —— 表现为「tsc 过、eslint 挂」；修法是把 helper 写成函数声明。P0-3-29 实测）。此前 2026-09-18 更新：「上线判据遇同形动态段路由时'没上'表现为 405 而非 404」「`npm run <script> --live` 不会把参数传给脚本、静默退化成纯断言」—— 要写 **`-- --live`**；**这类脚本的判据不能只看退出码**（失败时退出码同样是 0）。P0-3-28 复验时实测）。此前**§10.2 新增七行 P0-3-23 坑索引**：「**无凭证探针在受保护前缀下的页面路由上会失效**」（前缀匹配 + proxy 跑在路由匹配之前 → `/messages/任意路径` 恒 307，对照必须在受保护前缀**之外**；这类路由的上线判据只能用 build 路由表 / 真会话）、「探针里换过 token 的那个 client 不能再用来匿名读」（假阳性比漏报更危险）/ **配对判据必须是「同一性」不是「相似度」** / **多输入任务缺一个输入时应当整体失败**、不降级成"只有一半"的产物 / **站内路径也要守卫**（`//host` 是协议相对 URL）/ 中文文案里的 ASCII 直引号（三种形态 + 写作习惯）。此前**§10.2 新增六行 P0-3-20 坑索引**：「逻辑上永不成立」的守卫 = 判定结果没带回它的依据 / **自动化写入的行必须保持"未确认"**（否则功能在第一次接受之后就死了）/ 撤销里含 **update** 就不能按 id 删、必须按结构化旧值快照写回 / 客户端懒补**只换 payload 不整条替换** / 比时间戳**用 epoch 不用字符串** / 探针启发式正则**必须带词边界**）。此前 2026-09-17 更新（§10.1 新增第 21 条「同一判定在组件里分叉」+ 两行坑索引（分叉判定／Canvas 已判定完成时勾选框不可点）。此前同日更新含 §10.2 邮件入站相关坑索引累计新增：`wrangler secret put` 须在 deploy 之后 / 管道喂 secret 禁带换行 / catch-all 只能走专用端点 / `wrangler login` 后 Agent 可接管 Cloudflare 操作 / **一次改多个 Vercel env 会触发多个部署、中间那个先上线** / **审计表不是"邮件到没到"的判据**（无效 token 早退不落审计）/ **交互式召回阈值不能用于无人确认的自动落写** / **别用有副作用请求当部署探针（会打在旧部署真改数据）** / **"看起来安全"的探针输入也会命中（LLM 会剥后缀）**。更早 2026-09-13 更新含 §10.1 第 17–19 条与六行坑索引）*
