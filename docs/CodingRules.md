...
| 交互式「召回优先」阈值**不能**直接用于无人确认的自动落写 | `MATCH_THRESHOLD = 0.6` 是为**对话框列举候选**定的（多列一条用户划掉即可，召回优先合理）。但邮件入站**无人确认、直接写 `status='done'`** —— 同一阈值下 `Homework 9999`（不存在的作业）以 **0.84** 命中 `Homework 9`、`Homework 5` 与 `Homework 2/6/7` **同分 0.89**。根因：`normalizeTitle` 把数字也拼进 token 串，编辑距离 / Dice 对"数字被换掉"极不敏感；且 `matchTasks` 只按分数 `sort`，**同分无确定性 tie-break**（保留候选数组原序 = DB 返回顺序），**破坏了 match.ts 自己声明的"可复现"目标**。规则：**自动写路径要独立的高阈值（≈0.9 或要求归一化后完全相等）+ 确定性 tie-break，同分宁可放弃落写** | `EMAIL_INBOUND_SETUP.md` §8.3、`lib/email/plan.ts` |

---

*创建：2026-09-01 ｜ 最近更新：2026-09-17（§10.2 新增八行坑索引：`wrangler secret put` 须在 deploy 之后 / 管道喂 secret 禁带换行 / catch-all 只能走专用端点 / `wrangler login` 后 Agent 可接管 Cloudflare 操作 / **一次改多个 Vercel env 会触发多个部署、中间那个先上线** / **审计表不是"邮件到没到"的判据**（无效 token 早退不落审计）/ **交互式召回阈值不能用于无人确认的自动落写**。此前 2026-09-13 更新含 §10.1 第 17–19 条与六行坑索引）*
