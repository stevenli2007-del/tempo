...
| 2026-09-18 | **§12 新增「消息栏」+ `GET /api/v1/messages`、`PATCH /api/v1/messages/:id`**（P0-3-18）：系统提案收件箱 —— 读取返回全部状态、按时间倒序（处理完不消失，留「我点过什么」的凭据）；下发 `Message` 而非 `MessageView`，视图由客户端用同一个纯函数 `toMessageView()` 现算。PATCH 是**全站唯一能让提案生效的地方**：body `{ status: 'accepted' \| 'dismissed' }`，判定全收在 `planDecision()` 纯函数（忽略永不调 applier、确认只在 applier 就绪时改状态）；写入顺序刻意为「先改状态、再执行 applier，applier 失败尽力回滚 pending 并返 502」—— 反过来在 applier 有副作用时，状态更新失败 + 用户重试会把同一变更写两次 | P0-3-18、ADR-015 |
| 2026-09-18 | **§4 `grade_components` 的 `source` 契约补 `'canvas'`**（P0-3-24）：迁移 `20260918000000_grade_components_source_canvas.sql` 放宽 CHECK，Canvas 同步可写入成绩构成；端点形状无变化，仅 source 取值集同步标注 | P0-3-24 |
| 2026-09-18 | **§12 公告自动进站**（P0-3-25，ADR-022）：Canvas 公告在同步管线内进站，**无新端点** —— 口径为 C（有落点逐条建 `messages(type='announcement')` + 无落点合并为一条摘要），`partitionByLanding()` 是唯一判定点；去重键 `(course_id, canvas_announcement_id)`，`body_text` 只落清洗后纯文本；「知道了」式空落点确认沿用 §12 的 PATCH 语义 | P0-3-25、ADR-022 |
| 2026-09-18 | **§12 新增 `POST /api/v1/messages/summaries`**（P0-3-25b，ADR-024）：公告要点**懒生成** —— 打开消息栏后由客户端触发、**不进同步路径**、只对 `pending` 消息生成；结果落 `message_summaries`（`ok` / `failed` 都落行，failed 不重试），界面标「AI 总结」并保留「原文 ↗」外链 | P0-3-25b、ADR-024 |
| 2026-09-18 | **§12 `PATCH /api/v1/messages/:id` 扩撤销分支**（P0-3-26）：body `{ action: 'undo' }` —— 仅 `accepted` 且确认后 24h 内可撤销（超窗 403 `undo_window_expired`、非 accepted 409 `not_undoable`）；按 `applied` 结构化旧值回滚（考试行按 `examRestores` 还原、同步移除派生任务），撤销后 `status='undone'` 为终态；`decided_at` 列由迁移 `20260918000000_message_undo.sql` 落地。枚举「四处同改」相应扩 `undone` | P0-3-26 |
