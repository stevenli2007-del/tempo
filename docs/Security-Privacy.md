...
| 2 | **截图档额外告知**："截图会发送给视觉模型服务商（通义千问 Qwen，阿里云中国区 DashScope，无数据出境），请避免包含姓名 / 学号等个人信息"（P0-3-9 已写入设置页隐私说明） |
| 3 | 设置页列出当前使用的 provider 及其服务地区（文本 DeepSeek / 视觉 Qwen 通义千问，均中国境内） |
| 4 | **不含任何"数据不会被保存"的承诺** —— 无法保证的事不说（呼应 `CodingRules.md` 的诚实原则） |
| 5 | 若切换 provider，同步更新告知文案 |
| 6 | **邮件入站数据流向**（P0-3-11）：转发到 `inbound+<token>@<域>` 的邮件**正文**会发送给文本 LLM（DeepSeek，中国境内，符合已解决的 O-08）做解析；解析结果只用于「标记已提交任务为完成」，邮件正文**不留存**（仅入审计日志 `email_inbound_events`，含发件人/主题/事件，不含正文）。⚠️ 邮件可能含姓名 / 学号等个人信息，建议在转发前脱敏 |

**决策（记为待决事项 **O-08**，已于 2026-09-13 解决）**：采用 **全中国境内部署** —— 文本走 DeepSeek（中国）、视觉走通义千问 Qwen（阿里云中国区 DashScope）。两路数据均不离开中国境内，无数据出境合规风险。

| 选项 | 代价 | 状态 |
|---|---|---|
| ~~A. 维持现状（文本 DeepSeek + 截图 Claude）~~ | 数据出境（截图去美国 Anthropic），需用户知情 | ❌ 未采用 |
| ~~B. 全部改用美国境内 provider（Claude）~~ | 无出境到美国境外，但文本解析成本更高，且偏离"DeepSeek 优先" | ❌ 未采用 |
| ✅ **C. 全中国境内部署（文本 DeepSeek + 视觉 Qwen）** | 无出境；视觉能力由 Qwen VL 提供，不偏离 ADR-018 的按能力路由 | ✅ 已采用 |

> 选型依据（2026-09-13 Steven 拍板「A + Qwen」）：文本已是 DeepSeek（中国），只需把视觉从 Claude（美国）换成 Qwen（中国）即达成全境内部署，且不破坏 ADR-018 的按能力路由与"DeepSeek 优先"。
...
| A6 | 抓包确认前端所有响应不含 token | P0-2-2 | ✅ **2026-09-04 已验证（API 层）**：POST 201 / GET 200 / 诊断路由三类响应均断言不含 token 明文与 `secretEncrypted` 字段。**P0-2-4 补充（2026-09-04）**：关联 UI 的 SSR HTML 与全部 API 响应均已断言不含 token。⚠️ 仍未做的：**浏览器 DevTools Network 面板人工抓包**（`next dev` 在 WorkBuddy 起不来，Steven 浏览器验收只覆盖了功能正确性，未逐条看网络响应） |
...
| A11 | 删除账号级联清除 Storage 文件（不可遗漏） | P0-3-2 | ✅ **2026-09-09 已验证**：按 `{user_id}` 前缀递归删除（`list()` 只返一层且把文件夹当条目，须递归），自测含**孤儿文件**共 2 个对象全部删除；且**Storage 失败即整趟失败**，不会留下无主文件 |
| A12 | 设置页已含数据说明 + 明确告知 syllabus 会发送给 AI 服务商 | P0-3-2 | ✅ **2026-09-09 已验证**：设置页「我们存了什么」列课程 / 归档 / syllabus / 任务 / Canvas 连接态与 token 过期；隐私说明明确写出 syllabus 文本会发给 AI 服务商（当前 DeepSeek）且**其服务器不在美国境内**（关联开放问题 O-08） |
