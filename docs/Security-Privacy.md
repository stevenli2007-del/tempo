...
| 2 | **截图档额外告知**："截图会发送给视觉模型服务商（Anthropic，服务器在美国境内），请避免包含姓名 / 学号等个人信息"（P0-3-9 已写入设置页隐私说明） |
| 3 | 设置页列出当前使用的 provider 及其服务地区（文本 DeepSeek / 视觉 Claude） |
...
| 选项 | 代价 |
|---|---|
| **A. 维持现状（文本 DeepSeek + 截图 Claude），做好告知** | 数据出境，需用户知情；成本最低 |
| **B. 全部改用美国境内 provider（Claude）** | 无出境到美国境外，但文本解析成本更高，且偏离"DeepSeek 优先" |
| **C. 全部改用中国境内 / 本地模型** | 无出境，但视觉能力需另寻（DeepSeek 无视觉），且偏离 ADR-018 |
