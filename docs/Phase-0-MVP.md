...
| **P0-3-26** | **M3.5 · 回执 + 撤销**：`payload.before` 变更前快照 + 反向 applier（undoer）；`messages.status` 加 `'undone'`（枚举扩展须过 CodingRules §10.1 第 16 条：四处同改 types/message.ts/lib/messages/registry.ts/SQL CHECK/回归）；**撤销窗口 24h**（超窗只显历史）。⚠️ **开工前先想清与 3-25b 的交互**：要点只在 `status='pending'` 时显示 → 撤销后消息回到 `undone` 不是 `pending`，需明确"撤销后要不要恢复要点显示"（缓存行 `message_summaries` 不会被撤销删掉，命中即秒出，**不需要重新调模型**）；另外 `'undone'` 也算"已处理"，`canAccept`/`summaryPending` 都必须把它排除 | Bud | P0-3-25、P0-3-25b | ① 确认后出回执含原文链接 ② 24h 内可一键撤销还原 ③ 超窗不可撤销 | ⚪ |
