...
| **P0-3-25** | **M3.5 · 公告自动进站**：同步时批量端点 `GET /api/v1/announcements?context_codes[]=course_…` + **显式两端日期**（绕过 `end_date` 默认 +28 天陷阱）+ **窗口起点跟锚点走的动态滚动窗口**（常态"当天+昨天"，断更自动回补，14 天上限，ADR-023）；`course_announcements` 去重表 + HTML 清洗（禁 `dangerouslySetInnerHTML`）+ `html_url` 回跳；公告 → 进消息栏（复用 3-18）；**无结构化落点（如 office hours 变更）只给「知道了」回执，不写字段** | Bud | P0-3-24 | ① 公告进消息栏带原文链接 ② 有落点(quiz/exam 变更)→可确认提案 ③ 无落点→「知道了」④ 前置修订 Sync-Strategy §14 | ✅ **已上线**（`002522b` + C 口径 `07d810a` + 窗口收窄/课程色 `2bedddd`，Vercel success；迁移已跑 + 探针确认）。探针（2026-09-18，真账号）量出 48 条公告 / 40 条无落点 / 8 条判有落点但**只有 2 条真能写** → **Steven 拍板走 C（ADR-022）**：有落点逐条进 + 无落点合并成一条摘要；消息栏只新增 **9 条**（8 逐条 + 1 摘要）而一条不漏。**2026-09-18 晚追加验收修正（ADR-023）**：窗口收窄（锚点驱动，实测 48 条 → 5 条 → **真正新增 0 条**）+ 课程彩色徽标 | 🔵
| **P0-3-25b** | **M3.5 · 公告要点（AI 总结）**：给消息加中文要点，**不进同步路径** —— 懒生成 + 落库缓存（`message_summaries`，key `(message_id, locale)`）；打开消息栏后 `POST /api/v1/messages/summaries` 静默补齐；**只对仍待处理的公告消息**生成；覆盖不全如实标（"基于最新 20 条 / 共 40 条"）；失败落 `failed` 行 = 不再重试；**必须标「AI 总结」+ 保留「原文 ↗」**；语言是入参（先中文、course code / 考试名保留英文，英文版加一行数据即可） | Bud | P0-3-25 | ① 打开消息栏后公告气泡出现中文要点（标「AI 总结」+ 原文链接）② 刷新后**立刻**就有（缓存命中，不重算）③ 摘要消息标出覆盖率 ④ 模型挂了消息栏照常可用 | 🔵 **代码完成待验收**：迁移 `20260920000000`【Steven 手动】→ 代码 push → Vercel 部署。门禁 `tsc` ✓ / `eslint` 0 error / `build` ✓（新路由与 `[id]` 无冲突）；回归 `regress:message-summaries` **89 断言** + `regress:messages` **83**；在线探针 `probe:message-summaries`（真模型零写入）池化 40 条 → 4 条要点 / 单条精确 / **0 编造日期** / 0.86–1.34s。⚠️ **验收要先"回炉"一条消息**：3-25 的 9 条公告消息已全部处理完（1 确认 + 8 忽略）→ 没有 pending 就没有生成入口；把摘要那条改回 pending 再打开消息栏即可看到效果（`update messages set status='pending' where id='6bdfb787…'`，可逆） | ⚪ |
| **P0-3-26** | **M3.5 · 回执 + 撤销**：`payload.before` 变更前快照 + 反向 applier（undoer）；`messages.status` 加 `'undone'`（枚举扩展须过 CodingRules §10.1 第 16 条：四处同改 types/message.ts/view.ts/SQL CHECK/回归）；**撤销窗口 24h**（超窗只显历史） | Bud | P0-3-25 | ① 确认后出回执含原文链接 ② 24h 内可一键撤销还原 ③ 超窗不可撤销 | ⚪ |
