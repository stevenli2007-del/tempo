...
>
> ⏸ **两个产品问题待 Steven 拍板（2026-09-03 提出，未答，Bud 已停手）**：① **要不要包含「标记任务完成」**？要的话走契约 §5 的 `PATCH`（只允许改 status；考试派生任务的标题/日期不可改，改了返 422 引导去课程页）—— 这是 ADR-004 在接口层的强制点；② **已完成的任务在近期列表里怎么处理**？默认隐藏 + 「显示已完成（N）」开关 / 全部显示并置灰划掉 / 完全不显示。
...
> - 📌 **待 Steven 手动**：删 8 个测试账号（`p15b-a`、`p15b-b`、`p15b-prod`、`p15b-prod2`、`p16-a-*`、`p16-b-*`、`p18-a-*`、`p18-b-*`，均 `@test.dev`；业务数据已清，只剩 auth.users 里的壳）。
>
...
| **P0-1-9** | 总览页 v1：课程卡片（显示近期 1-2 个任务）+ 跨课程近期任务列表（仅 syllabus 数据，按日期排序） | Bud | P0-1-8 | 能看到"最近 7 天所有课程要做的事"；可跳转课程详情 | 🔵 未开工（两个产品问题待 Steven 拍板） |
