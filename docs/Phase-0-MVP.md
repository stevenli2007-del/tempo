...
| **P0-3-19** | **资料索引（Canvas 文件元数据）**：同步抓 `/files`+`/folders` **元数据** → `course_files` 表 → 课程页「资料」区按 Canvas 文件夹分组 + 外链回 Canvas；**只存目录不下载内容** | Bud | P0-3-27 | 资料按 Canvas 结构分组展示；外链回 Canvas；403 受限文件跳过不挂 sync | ⚪ |
