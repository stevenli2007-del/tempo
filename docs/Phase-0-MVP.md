...
#### 🎫 P0-3-14 · 主动提醒 / 优先级 🆕 已立项 2026-09-13（Steven 同意），待细化
...
- **🔴 与其他卡的边界（开工前必须先划清，否则两卡重复造）**：建议 **`P0-3-11` 只做入站**（转发地址 → 解析 → 更新任务，+ 一封「地址已就绪」验证邮件）；**出站推送体系（提醒时机 / 优先级 / 频控 / 退订）整体归本卡**。
  > ⚠️ **此边界尚未经 Steven 拍板** —— 3-11 的卡面目前仍写着"入站 + 出站"，落定后需同步改那张卡的表述。
- **待 Steven 拍板（本卡开工前，2 项）**：
  1. **提醒渠道**：**邮件**（复用 3-11 的入站基础设施，成本最低）/ **应用内横幅**（已有 P0-2-8 的过期提醒机制）/ 两者。Web Push 需新依赖 + 平台侧支持，**建议排除**。
  2. **「优先级」的口径**：按截止临近？按欠账量？按考试权重？—— **这决定本卡是"提醒"还是"建议"**，也决定要不要碰 Phase 2 的权重排序（`Roadmap.md` 已把 routine / 今日任务归 Phase 2）。若口径说不清，本卡应**缩到只做提醒**、把"优先级"留给 Phase 2。
