...
| **P0-3-27** | **M3.5 · URL 自动抓取**：贴链接 → 抓首页 → 取同源 `<a>` → 跟一层子页（≤5）→ 合并文本 → 复用 3-25 清洗；护栏：仅 http(s)/超时 5s/≤1MB/≤5 子页/**禁内网 SSRF**（169.254/10./127./172.16-31/192.168）；合并文本 → 复用 3-24 parse 通道自动设 course section card | Bud | P0-3-25、P0-3-26 | ① math53 berkeley 链接跟子页设 section card ② 内网地址拒绝 ③ 无 PDF 也能建卡 | ⚪ |
