...
**当前 task**：`P0-2-11` 总览页合并 🔵 **核对完成待 Steven 拍板 + 验收（2026-09-05 晚）** —— 执行卡见下方「P0-2-11 总览页合并」。依赖 P0-2-5、P0-1-9（均已验收）。**定位：合并机制已随 P0-1-9 + P0-2-5 落地，端到端核对 28/28 全绿、未发现功能缺陷；本卡交付是 2 处契约收口，另有一个来源标签问题待 Steven 拍板。**
...
**总览页要不要给 Canvas 作业加「作业」来源标签？** 现状：考试任务有「考试」标签（`isDerived` 驱动），
Canvas 作业无标签 —— 两类混排时，只有考试能被一眼认出。可选项：① 都加标签（对称，信息更全，列表更密）；
② 保持现状（考试是少数派，标出来即可，Canvas 作业靠课程名 + 标题自解释）；③ 只给 Canvas 作业加，考试不加。
Bud 倾向 ② —— Phase 0 列表只有几十条，且课程名已经区分了来源。
