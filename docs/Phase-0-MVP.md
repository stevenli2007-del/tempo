...
**当前 task**：`P0-3-10` Canvas 提交状态同步 + 日期时区修复 🔴 **地基卡，进行中**（领卡 2026-09-13；依赖 P0-3-3 ✅ / P0-3-5 ✅ / P0-3-6 ✅ 已收；代码已落盘，待 Steven 跑迁移 + 重同步验收）。
> ⚠️ **3-7 / 3-8 / 3-9 全部排在 3-10 之后**：可视化（3-7）依赖数据准确，截图档（3-9）依赖 O-11，都先让地基落地。
...
| **P0-3-7** | **总览页可视化**：周历 / 时间线视图 + 考试日期并入主 Calendar + 进度条（**分子分母口径必须先定**） | Bud | **P0-3-10** | 考试日期在日历可见；进度条口径明确且数据准确 | ⚪ |
...
| **P0-3-10** | **Canvas 提交状态同步 + 日期时区修复**：`include[]=submission` + 三态 + 「待确认」态 + 按学校时区渲染 | Bud | — | 见下方执行卡 5 条验收路径 | 🔵 进行中（领卡 2026-09-13） |
...
#### 🎫 P0-3-10 · Canvas 提交状态同步 + 日期时区修复 🔵 **进行中（领卡 2026-09-13）**
