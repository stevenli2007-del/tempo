...
| **P0-3-27** | **M3.5 · URL 自动抓取**：贴链接 → 抓首页 → 取同源 `<a>` → 跟一层子页（≤5）→ 合并文本 → 复用 3-25 清洗；护栏：仅 http(s)/超时 5s/≤1MB/≤5 子页/**禁内网 SSRF**（169.254/10./127./172.16-31/192.168）；合并文本 → 复用 3-24 parse 通道自动设 course section card。**三项拍板（2026-09-18）= A/A/A**：① 入口 = 对话框输入框**自动识别 URL**（零新 UI，复用 3-24 通道）；② `POST /api/v1/ingest/url` **只返回文本**（解析仍走现有 `/tasks/parse`，不新开解析路径）；③ 子页用**关键词启发式**（本地确定性、零 LLM） | Bud | P0-3-25、P0-3-26 | ① math53 berkeley 链接跟子页设 section card ② 内网地址拒绝 ③ 无 PDF 也能建卡 | 🟢 **代码完成 + 门禁全绿**（tsc / eslint 0 error / build / `regress:url-ingest` **67**）**本地自测通过，待 Steven 验收**；零迁移（完全复用 3-24 通道 + 3-25 清洗）；端到端验收需真链接（粘贴链接 → 解析 → 确认） |
