...
**当前 task**：`P0-3-7b` 总览页收纳（课程卡搬出 dashboard → 新建 `/courses` + 侧栏「我的课程」入口） 🟡 **代码完成 + 本地自测通过（待 Steven 验收）** —— 新增 `app/(routes)/courses/page.tsx` / `lib/courses/course-list.ts` / `lib/tasks/format.ts`；`/dashboard` 瘦身为纯任务视图（`<h1>` 改「接下来」）。**Steven 拍板两个决定**：① 侧栏**单入口**「我的课程」→ `/courses`（不内嵌课程列表）；② 课程卡**原样搬**、保持 2 列。**命名修正**：侧栏那项不是「单科详情」（该页早已存在于 `/courses/[id]`），而是课程目录。
...
后续可开：`P0-3-8`（课程更新对话框·文本档，无前置决策）/ `P0-3-9`（截图档，仍依赖 O-11）。
...
| **P0-3-7b** | **总览页收纳**：课程卡搬出 dashboard → 新建 `/courses` 列表页 + 侧栏「我的课程」入口（**结构改动，排在 3-12 视觉扫尾之前**） | Bud | P0-3-7 | dashboard 只回答「接下来要做什么」；课程资产独立成页 | 🟡 代码完成（待验收） |
| **P0-3-8** | **课程更新对话框（文本档）**：输入 + AI 解析 + **人工确认** + 只写手动任务 | Bud | P0-3-4 | 对话**不篡改** syllabus 权威数据（exam / 已确认项） | ⚪ |
...
#### 🎫 P0-3-7 · 总览页可视化：周历 / 时间线 + 考试并入 + 进度条 ✅ **代码完成（2026-09-13，待 Steven 验收）**
...
#### 🎫 P0-3-7b · 总览页收纳：课程卡搬出 dashboard + 新建 `/courses` 🟡 **代码完成（2026-09-13，待 Steven 验收）**
...
- **定位（ADR-016）**：**对话框是兜底，不是入口**。每加一个对话用例都要先问「这能不能自动化」——长期目标是让它被自动化（邮件 / 官方接口）取代。
- **硬约束**：
