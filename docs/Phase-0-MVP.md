...
| **P0-3-20** | **大纲漂移检测**：同步**零下载**只挑出 syllabus 文件（复用 3-19 的 `course_files` 元数据）→ 打开消息栏时**懒补 diff**（抽文本 → 与已确认五板块 + `exam_dates` 比对）→ 差异**提案进 `P0-3-18` 消息栏**（**不自动覆盖**） | Bud | P0-3-19、P0-3-27 | 抓到 syllabus 变化 → 出差异提案 → 确认才写 | 🟡 **代码完成 2026-09-18，等 Steven 验收**（`regress:syllabus-drift` 106/0 + `tsc`/`eslint`/`build` 全绿 + 真账号只读探针跑通）；**迁移 `20260923000000_syllabus_drift.sql` 已跑 + 两条腿复核生效**（截图 `FOREIGN KEY → course_files` + 探针「✓ 两列都在」）。⚠️ 标准 ①「抓到真变化 → 出提案」仍未被真实数据验过 |
...
#### 🎫 P0-3-20 · 大纲漂移检测 🟡 代码完成 2026-09-18（待 Steven 验收）
