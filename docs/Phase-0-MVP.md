...
**当前 task**：`P0-3-8b`（课程更新对话框·**检索/更新档**）—— **代码完成，待 Steven 浏览器验收**（3-8 + 3-8b 合并验收）。
- ✅ `P0-3-8` 课程更新对话框（文本档）**代码完成 + 已 push `45d3bc3`**：全局浮窗 FAB + `POST /api/v1/tasks`（手动任务）+ `DELETE /api/v1/tasks/:id` + `POST /api/v1/tasks/parse`（LLM 解析·**不落库**）。**用户验收 ⏳**。
- ✅ `P0-3-8b` 对话框**检索/更新档** **代码完成**：新增 `lib/tasks/match.ts`（**确定性匹配，不用 LLM**）+ `GET /api/v1/tasks/search`（候选检索）+ `PATCH` 放开手动任务内容编辑（非 manual 返回 `source_not_editable`）。浮窗流程升级为「解析 → **先检索现有任务** → 0 命中新增 / 有命中**列举让用户选** → 确认」。**用户验收 ⏳**。
...
后续可开：`P0-3-9`（截图档，仍依赖 O-11）/ `P0-3-11`（邮件通道，等 3-8 验收）/ 🆕 **建议立项「主动提醒 / 优先级」** —— 按 **ADR-017**，它是"秘书"最核心的护城河能力，目前无对应卡。
...
**M3 已重定义为「打磨收敛期」（2026-09-13）：13 张卡，出口门 = P0-3-13 版本 freeze，之后不追加。**
...
| **P0-3-2** | 设置与隐私页：数据清单 + 隐私说明 + 一键删 Canvas 凭据 + 一键删账号（级联） | Bud | P0-2-9 | 删除后所有相关数据（含 Storage 文件）被清除 | 🔵 代码完成 2026-09-09（自测 23/23，待验收） |
...
| **P0-3-8** | **课程更新对话框（文本档）**：输入 + AI 解析 + **人工确认** + 只写手动任务 | Bud | P0-3-4 | 对话**不篡改** syllabus 权威数据（exam / 已确认项） | ✅ 代码完成 2026-09-13（`45d3bc3`；待用户验收） |
| **P0-3-8b** | **对话框检索/更新档**：输入后**先检索现有任务** → 0 命中新增 / 有命中列举让用户选改哪条；`PATCH` 放开手动任务内容编辑 | Bud | P0-3-8 | 只有 `source='manual'` 可改（canvas/exam 被拦并引导源头）；匹配确定性、不用 LLM | ✅ 代码完成 2026-09-13（待用户验收） |
| **P0-3-9** | **课程更新对话框（截图档）**：多模态接入 | Bud | P0-3-8、O-11 | **验收用例：Chem 1A Homework 6 的 Gradescope 提交成功页截图 → 自动标记完成** | ⚪ 依赖 O-11 |
...
**排序纪律（不是编号顺序）**：`3-3 设计基线 → 3-4 / 3-5 / 3-6 结构与内容 → 3-10 地基（提交状态 + 时区）→ 3-7 可视化 → 3-8 / 3-9 / 3-11 对话框与通道 → 3-12 扫尾`。
...
#### 🎫 P0-3-8 · 课程更新对话框（文本档） ✅ **代码完成（2026-09-13，待用户验收）**
...
#### 🎫 P0-3-8b · 对话框「检索 / 更新」档（Follow-up） ✅ **代码完成（2026-09-13，待用户验收）**
...
- **自测**：`next build` 全过（类型 + 编译）；纯函数回归 **39/39**。运行时端到端烟测待 Steven 浏览器验收。

#### 🎫 P0-3-9 · 课程更新对话框（截图档） ⚪ 依赖 O-11

- **做什么**：截图丢进对话框 → 自动识别。
- **🔴 前置 O-11**：DeepSeek **无视觉**（`MEMORY.md` §关键技术决策），必须加多模态 provider（Claude / Gemini / GPT），ADR-003 抽象层需扩 `vision` 能力。备选：先 OCR 转文字再喂 DeepSeek（省钱，但丢版式 / 表格 / 邮件信息）。
- **验收用例（具体可点）**：**Chem 1A Homework 6 的 Gradescope 提交成功页截图 → 自动标记完成**。这比"能识别截图"可验得多，也是截图档的杀手级用例。
- **其他约束**：截图常含个人信息（邮件 / 姓名 / 学号）→ 隐私说明 A12 要更新；图片落 Storage 私有桶（P0-3-2 的删账号已按 `{user_id}` 前缀递归删，天然覆盖）；成本要计数（DeepSeek 曾触发 429 日限流）。
...
- **做什么**：ADR-014 的落地，hands-off 的**启用件**。
  - **入站**：转发到 Tempo 专属地址 → 解析 → 自动更新 task（Gradescope / Piazza / 课程通知 / lab 取消通知全覆盖，**完全不用碰这些平台**）。
  - **出站**：每日摘要 / 提醒推送（ADR-016 R4「Tempo 找人，不是人找 Tempo」）。
- **为什么现在做**：当前同步触发是"用户打开 dashboard"（T1）—— **用户不来，数据就不新，所以他必须来**，这是结构性矛盾。邮件入站把触发器换成外部事件。
- **关键约束**：零 OAuth（转发路径），成本远低于 Gmail API；Gmail 全自动留 Phase 1（restricted scope 需安全评估，周期以月计）。
