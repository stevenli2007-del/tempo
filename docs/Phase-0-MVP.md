...
| **P0-3-17** | **课程详情重排 + 抓取信息落地**：① 单列 `max-w-3xl` → 一屏看全 ② `null`(Canvas 不追踪)任务显灰「需手动确认」、移出逾期红+提醒 ③ 点任务名跳 Canvas(`html_url`，加列+映射) ④ 成绩构成→pie chart ⑤ `score/points_possible` bar，**只列已出分的**，折叠进「作业详情」 | Bud | P0-3-10、P0-3-15 | ① 详情页不滚动即看全 ② `none` 类不标红 ③ 点标题外链 Canvas ④ 饼图渲染 ⑤ **只列已出分**（没分的行不出现） | 🔵 已上线 2026-09-17 + 验收修正已改，**待复验** |
...
#### 🎫 P0-3-17 · 课程详情重排 + 抓取信息落地 🔵 已上线 2026-09-17（迁移已跑 + 部署 success；验收修正已改，待复验）
