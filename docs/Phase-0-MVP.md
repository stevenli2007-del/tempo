...
**当前 task**：`P0-3-3` 设计基线（移植 Stride 令牌 + 应用外壳）⚪ **未开工**。
...
| **P0-3-3** | **设计基线**：移植 Stride 设计令牌 + 应用外壳（固定侧栏 220px / 吸顶顶栏 76px / 卡片网格 / 深色主题） | Bud | P0-3-2 | 令牌进 Tailwind 配置；外壳在所有页面生效；深色主题可读 | ⚪ |
| **P0-3-4** | **解析质量修复**：`L1–L40` 讲座编号课表稳定漏抽（真 bug） | Bud | — | 该格式课表能被抽出 | ⚪ |
| **P0-3-5** | **模块与上传解耦**：空课程直接进五模块，上传降级为模块内可选动作 | Bud | P0-3-3 | 建课 → 直接看到五模块空态 → 可选上传 | ⚪ |
...
**排序纪律（不是编号顺序）**：`3-3 设计基线 → 3-4 / 3-5 / 3-6 / 3-7 结构与内容 → 3-12 扫尾`。
先定结构与内容、再上皮肤 —— 反过来做会把新界面**画两遍**。
🔴 **P0-3-10 是地基**：完成状态与日期都错着的时候做进度条（3-7）和激励，等于庆祝一件没发生的事（ADR-016）。
