...
| **P0-3-25** | **M3.5 · 公告自动进站**：同步时批量端点 `GET /api/v1/announcements?context_codes[]=course_…` + **显式两端日期**（绕过 `end_date` 默认 +28 天陷阱）+ 14 天滚动窗口幂等；`course_announcements` 去重表 + HTML 清洗（禁 `dangerouslySetInnerHTML`）+ `html_url` 回跳；公告 → 进消息栏（复用 3-18）；**无结构化落点（如 office hours 变更）只给「知道了」回执，不写字段** | Bud | P0-3-24 | ① 公告进消息栏带原文链接 ② 有落点(quiz/exam 变更)→可确认提案 ③ 无落点→「知道了」④ 前置修订 Sync-Strategy §14 | 🔵 **代码完成待验收**（`002522b` + C 口径改动；迁移已跑 + 探针确认）。探针（2026-09-18，真账号）量出 48 条公告 / 40 条无落点 / 8 条判有落点但**只有 2 条真能写** → **Steven 拍板走 C（ADR-022）：有落点逐条进 + 无落点合并成一条摘要**；消息栏只新增 **9 条**（8 逐条 + 1 摘要）而一条不漏。**待 push + Steven 验收** |
