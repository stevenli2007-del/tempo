...
**当前 task**：`P0-3-7` 总览页可视化（周历 + 考试并入 + 负担卡） 🟡 **代码完成 + 本地自测通过（待 Steven 验收）** —— 新增 `lib/time.ts` / `lib/tasks/progress.ts` / `components/overview/{week-calendar,debt-bar}.tsx` + `scripts/regress-progress.ts`（回归 17/17 PASS）。**开工前 Steven 拍板了三个决定**：① 负担条**只算 `source='canvas'`** 的任务；② 周历**滚动 7 天、今天在最左**；③ 待办清单保留在周历下方。
> ⚠️ **执行中偏离一处（已落文档）**：原计划口径文件叫「进度条」，实际做成**「已到期作业」负担卡** —— 因为分母会随老师发布作业而**自身膨胀**（全学期口径百分比会倒退），且没有工作量字段。它回答的是「欠了多少」，不是「完成多少」。见 `API-Contract.md §5.1`。
> ⚠️ **执行中追加一处**：7 天窗口下**一场考试都看不到**（实测 4 场考试全在 7 天之外，最近的还有 9 天）→ 追加「最近的考试」条（不受 7 天限制）。执行卡的「考试并入主 Calendar」靠它才真正兑现。
后续可开：`P0-3-8`（课程更新对话框·文本档，无前置决策）/ `P0-3-9`（截图档，仍依赖 O-11）。
> ⚠️ **3-7 / 3-8 / 3-9 原排在 3-10 之后**：3-10 已于 2026-09-13 完成验收 → **3-7 已解闸**、3-8（依赖 3-4 已完成）亦可开；截图档 3-9 仍依赖 O-11（多模态 provider），O-11 未解决前不要硬上。
...
| **P0-3-7** | **总览页可视化**：周历 / 时间线视图 + 考试日期并入主 Calendar + 进度条（**分子分母口径必须先定**） | Bud | —（已解闸：3-10 2026-09-13 完成） | 考试日期在日历可见；进度条口径明确且数据准确 | ✅ 代码完成（待验收） |
