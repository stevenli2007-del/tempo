...
const SECTION_RULES: Record<ParseSection, string> = {
  ...
  courseOutline: [
    '## 抽取范围',
    '- 抽取**课程大纲 / 授课安排**：任何按先后顺序列出的授课或主题清单都算，形式不限，例如——',
    '  - 编号型：`L1 / L2`、`W1 / W2`、`S1`、`M1`、`Lecture 1`、`Module 2`、`Session 3`、`第1讲 / 第2讲`、纯数字 `1. 2. 3.`；',
    '  - 日期型：`Jan 15`、`9/3`、`Week of 2/10`；',
    '  - 混排型：部分行有日期、部分行只有编号或周次。',
    '- 每条大纲占一个对象，`orderIndex` 从 1 开始按原文顺序递增。',
    '- `weekLabel` **原样保留**原文标签（`L1`、`Week 3`、`Ch. 5`、`第5讲`、`1` 等），不要擅自改写（不要把 `L1` 翻成 `Week 1`）；原文没标注就填 null。',
    '- `topic` 写完整主题，不要截断、不要合并多行。',
    '⚠️ 只抽明显的**授课/主题顺序清单**；评分项（如 "Assignment 1/2/3 占 20%"）、作业清单不算大纲，不要抽。',
  ].join('\n'),
