...
# 注：P0-3-35 / P0-1-11 之外的**基线后新增卡表行**（如 P0-3-35）不进 DECLARED_EDITS ——
# 该机制只作用于基线内已存在的行；新增行本身的后续状态修改属于「新增内容」的范畴。
...
DECLARED_NEW_CARDS: set[str] = {
    ...
    'P0-3-36',   # 2026-09-23 新开：公告/提案撞上「可能是同一场」时必须让人挑「覆盖哪条 / 新增」+ Canvas 空壳作业去重（同一类数据准确性 bug，Steven 拍板插队）
    # ── P0-5 功能批次（2026-09-23 Steven 拍板，插在 3-12/13 freeze 之前；理由：视觉 freeze 最后做，先上功能）──
    'P0-5-1',    # i18n 英文版（核心四页 + 语言切换；只动文案层）
    'P0-5-2',    # 课程详情页重构（Data C8 式左侧目录；只重构布局）
    'P0-5-3',    # 外部课程网站链接监控（Cron 重抓 + 比对进消息栏；ADR-026）
    'P0-5-4',    # 音符储值 + 彩带动画（轻反馈；ADR-016 / R5）
    'P0-5-5',    # 收尾遗留（公告 PDF 考试地点 / 公告改期路径 / 提醒邮件空壳过滤）
}
